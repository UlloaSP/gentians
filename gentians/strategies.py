...
class Strategy:
    ...
    def genetic_solver(
        ...
        def evaluate_score(
            stub_indexes: "list[int]", prog_indexes: "list[int]", program: "list[str]"
        ...
        def crossover(
            best_a: Individual, best_b: Individual
        ) -> "tuple[Individual,Individual]":
            """
                ...
                if random.random() < self.args.mutation_probability:
                    ...
                    if not change_stub:
                        ...
                        new_prog_pos = random.randint(
                            0, len(self.placed_list[new_stub].placed_clauses) - 1
                        )
                        # print(f"{new_program[i]} replaced with")
                        new_element.program[i] = self.placed_list[
                            new_stub
