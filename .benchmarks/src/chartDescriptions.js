export const CHART_DESCRIPTIONS = {
  ...
  "Porcentajes de tiempo por tipo":
    "Clauses suma clause generation. Evolution suma pregrounding, initialization, selection, crossover, mutation, replacement y search orchestration. Cada segmento es 100 × tiempo del tipo / tiempo del bloque, por lo que cada barra suma 100%. El tooltip muestra también los segundos y tiempo del bloque / total_execution.",
  "Progreso de búsqueda":
    "En una ejecución, cada punto usa la generación registrada: max es el mayor fitness de la población, avg su media y best so far el mejor valor observado hasta entonces. En vista media se alinean todas las generaciones, se arrastra hacia delante el último valor disponible por ejecución y se calcula la media; la banda muestra mínimo y máximo. best es el máximo de max entre ejecuciones en cada generación.",
  ...
  "Mejora operadores":
    "Las tasas se calculan por ejecución y después se promedian. En crossover y mutation, el denominador son resultados válidos con score original y nuevo; improved cuenta los que aumentan score. En replacement, el denominador es accepted + not competitive e improved compara candidato con víctima. worse/equal es el resto del mismo denominador.",
  "Impacto score operadores":
    "Media por ejecución del cambio de score y después media entre ejecuciones. Para crossover y mutation, delta = new_score − original_score en resultados válidos. Para replacement, delta = candidate_score − victim_score en reemplazos aceptados. Positivo indica mejora media; negativo, empeoramiento.",
  "Pérdida de ganancias de crossover":
    "Considera mutaciones aplicadas a descendientes cuyo crossover había producido una ganancia medible. perdida = casos en que la mutación elimina esa ganancia / eventos con ganancia de crossover; preservada = casos que la mantienen / el mismo denominador. Las tasas se calculan por ejecución y después se promedian.",
  ...
  "Modelos solve por etapa":
    "Para cada fase suma los modelos producidos por sus llamadas solve dentro de una ejecución y después promedia entre ejecuciones. El sector representa modelos medios de la fase / modelos medios de todas las fases. modelos/solve divide modelos medios entre llamadas solve medias del mismo grupo.",
